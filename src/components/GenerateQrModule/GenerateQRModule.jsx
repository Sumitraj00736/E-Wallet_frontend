...
export default function GenerateQRModule() {
  ...
  const handleGenerateQR = async () => {
    ...
    try {
      ...
      const qrCodeData = res.qrCodeData;
      setQrData({ id: qrCodeData, amount: Number(amount) });

      // Create QR code with the ID
      setQrCode(qrCodeData);
      console.log("QR Code Data set:", qrCodeData);

      // Connect WebSocket for QR status
      connectToQRStatus(qrCodeData, (data) => {
        console.log("WebSocket QR status update:", data);

        // Push notification
        setNotifications((prev) => [
          {
            id: qrCodeData + Date.now(),
            message: `Received ₹${data.amount} from ${data.payer}`,
            status: data.status,
          },
          ...prev,
        ]);
